...
colunas = ["AN_BASE", "NM_GRANDE_AREA_CONHECIMENTO", "SG_ENTIDADE_ENSINO", "NM_REGIAO", "SG_UF_PROGRAMA",
...
def lists(file):
    ...
    for i in range(min(list(file["ano"].unique())), max(list(file["ano"].unique())) + 1):
        dados = file.query(f"ano == {i}")
        dados = dados.query("grau_curso == 'DOUTORADO' or grau_curso == 'DOUTORADO PROFISSIONAL'")
    ...
    new_file.to_excel("/Users/pedroafmelo/Documents/projetos/sidtec_pb/etapa_2/pessoas_recursos/dados/cursos_posgrad_PB_agg.xlsx", index = False)
